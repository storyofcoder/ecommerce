...
import { addAddress } from "../../../apis/post/Index"
...
class Address extends React.Component {
    ...
    render() {
        let { new_address, add_address, selected_address, payment_mode } = this.state;
        let { user } = this.props;
                    ...
                    {(add_address == 1) &&
                        <div className="mt-3">
                            <form className="needs-validation">
                                ...
                                <div className="mb-3">
                                    <button className="btn w-100 wishlist_cart_btn" onClick={() => { this.addNewAddress() }}>Save Address</button>
                                </div>
                        ...
                        {delivery_address.map((value, index) => {
                            return <div className={`address_card ${selected_address == index ? 'address_card_active' : ''}`}>
                                <div className="row w-100" onClick={() => { this.props.selectedAddess(delivery_address[index]); this.setState({ selected_address: index }) }}>
                                    ...
                                    <div className="col-1">
                                        <span><i class="address_delete text-danger fa fa-trash" aria-hidden="true"></i></span>
                                    </div>
                                </div>
                            </div>
                        })}
