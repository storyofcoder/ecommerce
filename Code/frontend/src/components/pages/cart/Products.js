...
function Products(props) {
    ...
    }, [Object.keys(props?.cartList).length])
                                ...
                                return <li key={`cart_list_${index}`} className="items odd">
                                    <div className="infoWrap">
                                        <div className="cartSection">
                                            ...
                                            <h3>{value.title}</h3>
                                            <p> <input id={`cart_list_id_${index}`}  type="text" value={value.quantity} onChange={(e)=>updateCart(e.target.value, value.asin)} className="qty" placeholder="3" /> x {value?.price?.symbol}{value?.price?.current_price * (value.quantity || 1)}</p>
                                            <p className="stockStatus"> In Stock</p>
                                        </div>
                                        <div className="prodTotal cartSection">
                                            <p>{value?.price?.symbol}{value?.price?.current_price}</p>
                                        </div>
                                        <div className="cartSection removeWrap" onClick={() => handleAddCart(value.asin, 0)}>
